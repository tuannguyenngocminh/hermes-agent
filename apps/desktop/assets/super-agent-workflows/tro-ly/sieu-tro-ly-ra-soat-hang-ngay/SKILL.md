...
Trước khi viết báo cáo, phải rà soát lịch sử hội thoại theo đúng `## Review scope (system-generated)` được đính kèm trong prompt. Đây là thuật toán bắt buộc, không dùng số session hiển thị ở sidebar làm bằng chứng coverage:

1. **Baseline:** khi `mode: baseline`, bắt đầu với `session_search(limit=10, offset=0)`. Đọc kết quả của mọi trang; nếu `has_more` là `true`, gọi lại bằng nguyên `limit` và `next_offset` cho tới khi `has_more` là `false`. Với mỗi session trong phạm vi, đọc nội dung cần thiết bằng `session_search(session_id=...)` trước khi kết luận. Không dừng ở ba session đầu và không bỏ qua trang giữa.
2. **Incremental:** khi `mode: incremental`, chỉ dùng mốc `last_active_after` dạng Unix đã được hệ thống inject sẵn, không tự tính lại và không đổi mốc. Gọi `session_search` với đúng mốc đó, xử lý pagination như baseline và đọc từng session trong tất cả các trang thuộc phạm vi. Mốc này đã bao gồm lookback 7 ngày và chỉ áp dụng cho profile cron hiện tại.
3. **Coverage:** chỉ khi mọi trang đã được xử lý, mọi session cần thiết đã được đọc và không có lỗi truy xuất mới trả dòng đầu tiên non-empty là `B1_REVIEW_COVERAGE_COMPLETE`. Khi coverage đã hoàn tất nhưng không có gì đáng báo, trả đúng hai dòng: `B1_REVIEW_COVERAGE_COMPLETE` rồi `[SILENT]`. Nếu thiếu trang, thiếu session, không đọc được nội dung, profile không xác định hoặc có lỗi khác, trả dòng đầu tiên non-empty là `B1_REVIEW_COVERAGE_INCOMPLETE`, sau đó nói rõ giới hạn và phần chưa kiểm tra. Khi coverage chưa hoàn tất, **không được trả [SILENT]**.
4. Sau khi coverage hoàn tất, mới đọc bằng `read_file` ở chế độ chỉ đọc `${HERMES_SKILL_DIR}/../../../memories/USER.md` và `${HERMES_SKILL_DIR}/../../../memories/MEMORY.md`. Hermes thay `${HERMES_SKILL_DIR}` bằng đường dẫn tuyệt đối của thư mục skill trong profile đang chạy, nên hai đường dẫn này trỏ tới đúng bộ nhớ của profile đó mà không cần gọi `terminal`. Đây là các nguồn dữ liệu chính để hiểu công việc, trình độ và nhu cầu thực tế của người dùng. Chỉ gọi `dashboard_summary` đúng một lần nếu cần số liệu Dashboard cụ thể; không coi kết quả đó là thay thế cho lịch sử và hai file bộ nhớ.
