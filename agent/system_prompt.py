...
def _prompt_path(path: Path) -> str:
    """Render a prompt path portably without rewriting Windows drive paths."""
    rendered = str(path)
    return rendered.replace("\\", "/") if not path.drive else rendered
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if _agent_home_path is not None:
        ...
        _root_str = _prompt_path(get_default_hermes_root())
    else:
        _home_str = _root_str = _prompt_path(get_hermes_home())
